...
fn should_cache(dict_path: &Path, cache_path: &Path) -> Result<bool> {
    ...
    let (dict_meta, cache_meta) = (
        dict_path.metadata().wrap_err("failed to read dictionary file metadata")?,
        cache_path.metadata().wrap_err("failed to read cache file metadata")?,
    );
...
fn load_dictionary() -> Result<Trie<u8, model::Entry>> {
    ...
    match should_cache(&path, &cache_path)? {
        ...
        false => {
            ...
            save_to_cache(&trie, &cache_path).wrap_err("failed to save the dictionary word index")?;
            Ok(trie)
        }
    }
}
...
fn main() -> Result<()> {
    ...
    let res: Vec<(String, &Entry)> = dict.predictive_search(query).collect();
    for (_, entry) in res {
        println!("{entry:?}");
    }
